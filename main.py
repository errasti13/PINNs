...
from problem.NavierStokes import SteadyNavierStokes2D
from plots import *

def main():
    eq = 'NavierStokes'
    
    pinn = PINN(NOutputs=3)
    ...
    equations = {
        ...
        'NavierStokes': (SteadyNavierStokes2D, (-1, 1), (-1, 1), 'random')
    }
    ...
    if eq == 'Heat' or eq == 'NavierStokes':
        data = equation.generate_data(x_range, y_range=(-1, 1), N0=100, Nf=1000, sampling_method=sampling_method)
    else:
        data = equation.generate_data(x_range, t_range, N0=100, Nf=10000, sampling_method=sampling_method)

    pinn.train(equation.loss_function, data, epochs=10000, print_interval=100)
    ...
    if eq == 'Heat':
        ...
        uPred, vPred, pPred, X_pred, Y_pred = equation.predict(pinn, x_range, (-1, 1))
